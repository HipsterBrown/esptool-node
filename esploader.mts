import { type Data, deflate, Inflate } from "pako";
import { ESPError } from "./error.mjs";
import type { Transport } from "./webserial.mjs";
// import { Transfer } from 'usb';
import { } from 'serialport';
import type { ROM } from "./targets/rom.mjs";

async function magic2Chip(magic: number): Promise<ROM | null> {
  switch (magic) {
    case 0x00f01d83: {
      const { ESP32ROM } = await import("./targets/esp32.mjs");
      return new ESP32ROM();
    }
    case 0x6921506f:
    case 0x1b31506f: {
      const { ESP32C3ROM } = await import("./targets/esp32c3.mjs");
      return new ESP32C3ROM();
    }
    case 0x09: {
      const { ESP32S3ROM } = await import("./targets/esp32s3.mjs");
      return new ESP32S3ROM();
    }
    case 0x000007c6: {
      const { ESP32S2ROM } = await import("./targets/esp32s2.mjs");
      return new ESP32S2ROM();
    }
    case 0xfff0c101: {
      const { ESP8266ROM } = await import("./targets/esp8266.mjs");
...
export class ESPLoader {
  ...
  ui8ToBstr(u8Array: Uint8Array) {
    ...
    for (let i = 0; i < u8Array.length; i++) {
      b_str += String.fromCharCode(u8Array[i] ?? 0);
    }
    return b_str;
  }
  ...
  async command(
    ...
    if (op != null) {
      ...
      pkt[0] = 0x00;
      pkt[1] = op;
      pkt[2] = this._short_to_bytearray(data.length)[0] ?? 0;
      pkt[3] = this._short_to_bytearray(data.length)[1] ?? 0;
      pkt[4] = this._int_to_bytearray(chk)[0] ?? 0;
      pkt[5] = this._int_to_bytearray(chk)[1] ?? 0;
      pkt[6] = this._int_to_bytearray(chk)[2] ?? 0;
      pkt[7] = this._int_to_bytearray(chk)[3] ?? 0;
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const resp = p[0];
      const op_ret = p[1];
      const val = this._bytearray_to_int(p[4] ?? 0, p[5] ?? 0, p[6] ?? 0, p[7] ?? 0);
  ...
  async _connect_attempt(mode = "default_reset", esp32r0_delay = false) {
    ...
    if (mode !== "no_reset") {
      if (this.transport.get_pid() === this.USB_JTAG_SERIAL_PID.toString()) {
  ...
  async run_spiflash_command(spiflash_command: number, data: Uint8Array, read_bits: number) {
    ...
    if (data_bits == 0) {
      ...
      for (i = 0; i < data.length - 4; i += 4) {
        val = this._bytearray_to_int(data[i] ?? 0, data[i + 1] ?? 0, data[i + 2] ?? 0, data[i + 3] ?? 0);
  ...
  async run_stub() {
    ...
    let decoded = Buffer.from(this.chip.ROM_TEXT, 'base64').toString();
    let chardata = decoded.split("").map(function(x) {
      return x.charCodeAt(0);
    });
    const text = new Uint8Array(chardata);

    decoded = Buffer.from(this.chip.ROM_DATA, 'base64').toString();
  ...
  _update_image_flash_params(
    ...
    const magic = parseInt(image[0] ?? '0');
    let a_flash_mode = parseInt(image[2] ?? '0');
    const flash_size_freq = parseInt(image[3] ?? '0');
  ...
  async write_flash(
    ...
    if (flash_size !== "keep") {
      ...
      for (const file of fileArray) {
        if (file.data.length + file.address > flash_end) {
          throw new ESPError(`File ${file.address} doesn't fit in the available flash`);
        }
      }
    }
    ...
    for (const file of fileArray) {
      const index = fileArray.findIndex(f => f.address === file.address);
      this.debug("Data Length " + file.data.length);
      image = file.data;
      const reminder = file.data.length % 4;
      ...
      if (reportProgress) reportProgress(index, 0, totalBytes);
      ...
      while (image.length > 0) {
        ...
        seq++;
        if (reportProgress) reportProgress(index, bytes_sent, totalBytes);
      }
      if (this.IS_STUB) {
