...
class Transport {
  ...
  get_info() {
    // @ts-expect-error need a different type for device info
    const info = this.device.getInfo() as PortInfo;
    return info.vendorId && info.productId
      ? `WebSerial VendorID 0x${info.vendorId.toString()} ProductID 0x${info.productId.toString()}`
      : "";
  }

  get_pid() {
    // @ts-expect-error need a different type for device info
    return (this.device.getInfo() as PortInfo).productId;
  }
  ...
  async setRTS(state: boolean) {
    // @ts-expect-error no setSignals on serialport
  ...
  async connect(baud = 115200) {
    // @ts-expect-error device should already be open with baudrate set
  ...
  async waitForUnlock(timeout: number) {
    // not sure how to wait for unlock with serialport
    // while (
