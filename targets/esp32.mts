...
export class ESP32ROM extends ROM {
  ...
  TEXT_START = ESP32_STUB.text_start;
  ENTRY = ESP32_STUB.entry;
  DATA_START = ESP32_STUB.data_start;
  ROM_DATA = ESP32_STUB.data;
  ROM_TEXT = ESP32_STUB.text;
  ...
  async get_chip_description(loader: ESPLoader) {
    ...
    if (pkg_version >= 0 && pkg_version <= 6) {
      chip_name = chip_desc[pkg_version] ?? "Unknown ESP32";
    } else {
